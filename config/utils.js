...
exports.removePlugin = (name, plugins) =>
  plugins.filter(plugin => !(Array.isArray(plugin) ? plugin.includes(name) : plugin === name));

const baseDir = (exports.baseDir = (...args) => resolve(__dirname, '..', ...args));
exports.configDir = (...args) => baseDir(join('config', ...args));
exports.userFirebaseConfig = () => resolve(homedir(), '.config/configstore/firebase-tools.json');
